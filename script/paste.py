...
class paste(QMessageBox):
    def execute(self, script_variable):
        ...
        urls = mimeData.urls()
        for u in urls:
            src = u.path()      # 欲粘贴文件的源路径
            dst = join(cur_path, basename(src))     # 合成欲粘贴文件的目标路径
    ...
    def copy_file(self, is_exists, src, dst):
        if is_exists:
            reply = self.information(self, "确认文件替换",   "该位置已存在一个同名的文件" + basename(src),
    ...
    def copy_dir(self, is_exists, src, dst):
        if is_exists:
            reply = self.information(self, "确认文件夹替换",   "该位置已存在一个同名的文件夹" + basename(src),
                                     QMessageBox.Yes | QMessageBox.No)
            if reply == QMessageBox.Yes:
